...
class KalmanFilterXYAH:
    """
    这个类用于跟踪边界框在图像空间中的位置和速度，其中边界框的状态空间为 8 维 (x, y, a, h, vx, vy, va, vh)，包含边界框中心位置 (x, y)、宽高比 a、高度 h 以及它们的速度。
    ...
    def __init__(self):
        """初始化了卡尔曼滤波器模型矩阵，并设置了运动和观测不确定性的权重。Initialize Kalman filter model matrices with motion and observation uncertainty weights."""
        ndim, dt = 4, 1.0
    ...
    def initiate(self, measurement: np.ndarray) -> tuple:
        """
        用于从未关联的测量中创建跟踪，返回新跟踪的均值向量和协方差矩阵。
    ...
    def predict(self, mean: np.ndarray, covariance: np.ndarray) -> tuple:
        """
        运行卡尔曼滤波器的预测步骤，返回预测状态的均值向量和协方差矩阵。
    ...
    def project(self, mean: np.ndarray, covariance: np.ndarray) -> tuple:
        """
        将状态分布投影到测量空间中，返回给定状态估计的投影均值和协方差矩阵。
