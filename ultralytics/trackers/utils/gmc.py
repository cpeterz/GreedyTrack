...
#实现了一个名为 Generalized Motion Compensation (GMC) 的类，用于在视频帧中进行对象跟踪和检测。GMC 类提供了几种跟踪算法的方法，包括 ORB、SIFT、ECC 和稀疏光流，并支持对帧进行降采样以提高计算效率。
import copy
...
class GMC:
    ...
    Attributes:
        method：用于跟踪的方法，可选项包括 'orb'、'sift'、'ecc'、'sparseOptFlow' 和 'none'。
        downscale：用于处理帧的降采样因子。
        prevFrame：存储上一帧用于跟踪的图像。
        prevKeyPoints：存储上一帧的关键点。
        prevDescriptors：存储上一帧的描述符。
        initializedFirstFrame：指示是否已处理了第一帧的标志。
    ...
    Methods:
        __init__(self, method='sparseOptFlow', downscale=2)：初始化 GMC 对象，可以指定方法和降采样因子。
        apply(self, raw_frame, detections=None)：应用所选方法到原始帧上，可选地使用提供的检测结果。
        applyEcc(self, raw_frame, detections=None)：对原始帧应用 ECC 算法。
        applyFeatures(self, raw_frame, detections=None)：对原始帧应用基于特征的方法，如 ORB 或 SIFT。
        applySparseOptFlow(self, raw_frame, detections=None)：对原始帧应用稀疏光流方法。
        reset_params(self)：重置参数，用于重新初始化跟踪。
