...
# Settings
# 设置 PyTorch、NumPy 和 OpenCV 的打印和线程选项。
torch.set_printoptions(linewidth=320, precision=4, profile="default")
np.set_printoptions(linewidth=320, formatter={"float_kind": "{:11.5g}".format})  # format short g, %precision=5
cv2.setNumThreads(0)  # prevent OpenCV from multithreading (incompatible with PyTorch DataLoader)
# 设置了 NumExpr 和 CuBLAS 的环境变量，用于控制它们的线程数量和工作空间大小。
os.environ["NUMEXPR_MAX_THREADS"] = str(NUM_THREADS)  # NumExpr max threads
os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"  # for deterministic training
# 设置 TensorFlow 的日志级别，用于抑制冗长的警告消息。
os.environ["TF_CPP_MIN_LOG_LEVEL"] = "2"  # suppress verbose TF compiler warnings in Colab

class TQDM(tqdm_original):
    """
    用于在 Ultralytics 中控制进度条的显示。 Custom Ultralytics tqdm class with different default arguments.
...
class SimpleClass:
    """提供了有用的字符串表示、错误报告和属性访问方法，用于更轻松地进行调试和使用。 Ultralytics SimpleClass is a base class providing helpful string representation, error reporting, and attribute
    access methods for easier debugging and usage.
    """

    def __str__(self):
        """返回对象的人类可读字符串表示形式 Return a human-readable string representation of the object."""
        attr = []
        for a in dir(self):
            v = getattr(self, a)
    ...
    def __repr__(self):
        """返回对象的机器可读字符串表示形式 Return a machine-readable string representation of the object."""
        return self.__str__()

    def __getattr__(self, attr):
        """访问不存在的属性时提供自定义的错误消息 Custom attribute access error message with helpful information."""
...
def plt_settings(rcparams=None, backend="Agg"):
    """
    这是一个装饰器函数，用于临时设置绘图函数的 matplotlib rc 参数和后端。可以将其应用于任何需要特定 matplotlib rc 参数和后端的函数。 Decorator to temporarily set rc parameters and the backend for a plotting function.
