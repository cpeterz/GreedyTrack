...
# 用于评估目标检测模型的性能,还实现了很多IOU计算方法。具体来说，它提供了计算精确率、召回率、F1 分数、平均精度等指标的工具，以及绘制相关曲线（如 Precision-Recall 曲线）的功能。
import math
import warnings
...
def bbox_ioa(box1, box2, iou=False, eps=1e-7):
    ...
    # 先确保box1和box2是numpy数组且形状为(n, 4)和(m, 4)
    # 扩大边框
    b1_center_x = (b1_x1 + b1_x2) / 2
    b1_center_y = (b1_y1 + b1_y2) / 2
    enlarged_b1_x1 = (b1_center_x - (b1_x2 - b1_x1)).clip(0)
    enlarged_b1_y1 = (b1_center_y - (b1_y2 - b1_y1)).clip(0)
    enlarged_b1_x2 = (b1_center_x + (b1_x2 - b1_x1)).clip(0)
    enlarged_b1_y2 = (b1_center_y + (b1_y2 - b1_y1)).clip(0)

    b2_center_x = (b2_x1 + b2_x2) / 2
    b2_center_y = (b2_y1 + b2_y2) / 2
    enlarged_b2_x1 = (b2_center_x - (b2_x2 - b2_x1)).clip(0)
    enlarged_b2_y1 = (b2_center_y - (b2_y2 - b2_y1)).clip(0)
    enlarged_b2_x2 = (b2_center_x + (b2_x2 - b2_x1)).clip(0)
    enlarged_b2_y2 = (b2_center_y + (b2_y2 - b2_y1)).clip(0)

    enlarged_inter_area = (np.minimum(enlarged_b1_x2[:, None], enlarged_b2_x2) - np.maximum(enlarged_b1_x1[:, None], enlarged_b2_x1)).clip(0) * (
        np.minimum(enlarged_b1_y2[:, None], enlarged_b2_y2) - np.maximum(enlarged_b1_y1[:, None], enlarged_b2_y1)
    ).clip(0)
    print(f"enlarged_inter_area:{enlarged_inter_area}")

    enlarged_area = (enlarged_b2_x2 - enlarged_b2_x1) * (enlarged_b2_y2 - enlarged_b2_y1)
    if iou:
        enlarged_box1_area = (enlarged_b1_x2 - enlarged_b1_x1) * (enlarged_b1_y2 - enlarged_b1_y1)
        enlarged_area = enlarged_area + enlarged_box1_area[:, None] - enlarged_inter_area
        print(f"enlarged_area:{enlarged_area}")

    # Intersection over box2 area
    return enlarged_inter_area / (enlarged_area + eps)
