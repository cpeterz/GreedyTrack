...
# 这段代码包含了一些与目标检测相关的函数，主要涉及到生成锚框、计算锚框和真实框之间的重叠、以及处理正样本和负样本等操作。

def select_candidates_in_gts(xy_centers, gt_bboxes, eps=1e-9):
    """
    从锚点中选择正样本。该函数根据真实框和锚点的位置，选择正样本锚点，
    返回一个布尔型的 mask，表示每个锚点是否是正样本。Select the positive anchor center in gt.
...
def select_highest_overlaps(mask_pos, overlaps, n_max_boxes):
    """
    如果一个锚框被分配给多个真实框，则选择与之重叠最高的真实框。该函数根据每个锚框与所有真实框的重叠情况，选择与之重叠最高的真实框，并返回对应的索引。If an anchor box is assigned to multiple gts, the one with the highest IoI will be selected.
...
def generate_anchors(feats, fpn_strides, grid_cell_size=5.0, grid_cell_offset=0.5,  device='cpu', is_eval=False, mode='af'):
    '''从特征图中生成锚框。Generate anchors from features.'''
    anchors = []
    anchor_points = []
    stride_tensor = []
    num_anchors_list = []
...
def bbox_overlaps(bboxes1, bboxes2, mode='iou', is_aligned=False, eps=1e-6):
    """计算两组边界框之间的重叠。 Calculate overlap between two set of bboxes.
...
def iou2d_calculator(bboxes1, bboxes2, mode='iou', is_aligned=False, scale=1., dtype=None):
    ...
    """用于计算 2D 边界框之间的重叠度 Calculate IoU between 2D bboxes.
...
def dist_calculator(gt_bboxes, anchor_bboxes):
    """计算所有边界框和真实边界框之间的中心距离。 compute center distance between all bbox and gt
...
def iou_calculator(box1, box2, eps=1e-9):
    """计算批次中边界框的 IoU。 Calculate iou for batch
...
class ATSSAssigner(nn.Module):
    '''自适应训练样本选择分配器，实现了 ATSS 的分配逻辑。 Adaptive Training Sample Selection Assigner'''
    def __init__(self,
                 topk=9,
                 num_classes=80):
