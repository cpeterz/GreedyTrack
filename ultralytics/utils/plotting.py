...
# 定义了一些颜色相关的方法，包括将十六进制颜色代码转换为 RGB 值。
class Colors:
    """
...
# 主要用于在图像上绘制各种标注，包括边界框、文本、关键点等。
#
# 初始化时，可以选择使用 PIL 或 cv2 进行标注。
# 提供了绘制边界框、文本、关键点、计数等功能的方法。
# 包含一些私有方法，用于实际的绘图操作。
class Annotator:
    """
    ...
    # font：用于文本标注的字体。
    # lw：绘制线条的宽度。
    # skeleton：关键点的骨架结构，用于绘制人体姿态。
    # limb_color
    # 和
    # kpt_color：分别为关键点和骨架的颜色调色板。
    ...
    def kpts(self, kpts, shape=(640, 640), radius=5, kpt_line=True):
        ...
        if self.pil:
            # Convert to numpy first
            self.im = np.asarray(self.im).copy()
        nkpt, ndim = kpts.shape  #其中 nkpt 是关键点的数量，ndim 是每个关键点的维度。
    ...
    # 添加文本到图像的方法，支持PIL OpenCV（cv2）两种库。
    ...
    def draw_region(self, reg_pts=None, color=(0, 255, 0), thickness=5):
        ...
        Draw region line
        Args:
            reg_pts (list):区域的点集，对于线条而言是两个点，对于区域则是四个点。 Region Points (for line 2 points, for region 4 points)
    ...
    def draw_centroid_and_tracks(self, track, color=(255, 0, 255), track_thickness=2):
        ...
        Args:
            track (list): 目标的追踪点列表，用于显示轨迹。object tracking points for trails display
    ...
    def count_labels(self, counts=0, count_txt_size=2, color=(255, 255, 255), txt_color=(0, 0, 0)):
        ...
        Args:
            counts (int): 对象计数的值。objects counts value
    ...
    def estimate_pose_angle(a, b, c):
        """计算姿势的角度 Calculate the pose angle for object
        Args:
    ...
    def draw_specific_points(self, keypoints, indices=[2, 5, 7], shape=(640, 640), radius=2):
        """
        绘制特定的关键点 Draw specific keypoints for gym steps counting.
    ...
    def plot_angle_and_count_and_stage(self, angle_text, count_text, stage_text, center_kpt, line_thickness=2):
        """
        方法用于绘制姿势角度、计数值和步骤阶段 Plot the pose angle, count value and step stage.

        Args:
            angle_text (str): 姿势角度的文本值 angle value for workout monitoring
            count_text (str): 计数值的文本值 counts value for workout monitoring
            stage_text (str): 步骤阶段的文本值 stage decision for workout monitoring
            center_kpt (int): 姿势中心点的坐标 centroid pose index for workout monitoring
    ...
    def seg_bbox(self, mask, mask_color=(255, 0, 255), det_label=None, track_label=None):
        """
        绘制带有边界框形状的分割对象 Function for drawing segmented object in bounding box shape.
    ...
    def plot_distance_and_line(self, distance_m, distance_mm, centroids, line_color, centroid_color):
        """
        在图像上绘制出距离值和质心之间的连线，并在图像的左上角显示出距离的文本信息 Plot the distance and line on frame.
    ...
    def visioneye(self, box, center_point, color=(235, 219, 11), pin_color=(255, 0, 255), thickness=2, pins_radius=10):
        """
        在图像上绘制出视觉眼点，并将视觉眼点与边界框的中心点之间连接起来 Function for pinpoint human-vision eye mapping and plotting.
...
def plot_labels(boxes, cls, names=(), save_dir=Path(""), on_plot=None):
    """绘制训练标签，包括类别直方图和框统计信息。 Plot training labels including class histograms and box statistics."""
    import pandas as pd
...
def save_one_box(xyxy, im, file=Path("im.jpg"), gain=1.02, pad=10, square=False, BGR=False, save=True):
    """
    从图像中裁剪出一个包含目标区域的部分，并将其保存到文件中 Save image crop as {file} with crop size multiple {gain} and {pad} pixels. Save and/or return crop.
...
def plot_images(
    ...
    """绘制带有标签的图像网格 Plot image grid with labels."""
            ...
            if len(bboxes):
                ...
                for j, box in enumerate(boxes.astype(np.int64).tolist()):
                    ...
                    if labels or conf[j] > 0.25:  # 0.25 conf thresh
                        ...
                        annotator.box_label(box, None, color=color, rotated=is_obb)
...
def plot_results(file="path/to/results.csv", dir="", segment=False, pose=False, classify=False, on_plot=None):
    """
    绘制训练结果，从一个结果 CSV 文件中读取数据并绘制不同类型的图表，支持分割、姿势估计和分类等不同类型的数据 Plot training results from a results CSV file. The function supports various types of data including segmentation,
...
def plt_color_scatter(v, f, bins=20, cmap="viridis", alpha=0.8, edgecolors="none"):
    """
   绘制一个散点图  Plots a scatter plot with points colored based on a 2D histogram.
...
def plot_tune_results(csv_file="tune_results.csv"):
    """
    绘制调参结果的散点图和折线图。 Plot the evolution results stored in an 'tune_results.csv' file. The function generates a scatter plot for each key
...
def output_to_target(output, max_det=300):
    """将模型输出转换为目标格式，用于绘制。 Convert model output to target format [batch_id, class_id, x, y, w, h, conf] for plotting."""
    targets = []
...
def feature_visualization(x, module_type, stage, n=32, save_dir=Path("runs/detect/exp")):
    """
    可视化模型特征图 Visualize feature maps of a given model module during inference.
